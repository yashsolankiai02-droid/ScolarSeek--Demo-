      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-8 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="shrink-0 flex items-center space-x-4">
           <div className="text-right">
              ...
              <div className="text-xs text-gray-500 dark:text-slate-400">{completionPercentage}% Completed</div>
           </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
                <ul className="divide-y divide-gray-100 dark:divide-slate-800">
                  {pendingActions.map((action, idx) => (
                    <li key={idx}>
                      <Link to={action.link} className="flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 rounded-full bg-brand-50 flex items-center justify-center shrink-0">
                            {action.icon}
                          </div>
                          <span className="text-sm font-medium text-gray-900 dark:text-white">{action.msg}</span>
                        </div>
          ...
          <section>
            ...
            {savedScholarships.length === 0 ? (
               <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-8 text-center shadow-sm">
                 <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">You have no saved scholarships.</p>
          ...
          <section>
            ...
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-5 shadow-sm space-y-4">
              {applications.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-slate-400 text-center py-4">No active applications.</p>
              ) : (
                applications.map(app => (
                  <div key={app.id} className="flex justify-between items-center border-b border-gray-100 dark:border-slate-800 last:border-0 pb-3 last:pb-0">
                    <div className="truncate pr-4">
                      <div className="text-sm font-bold text-gray-900 dark:text-white truncate">{app.scholarshipName}</div>
                      <div className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{app.status}</div>
                    </div>
                  </div>
                ))
              )}
